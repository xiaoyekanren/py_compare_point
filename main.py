...
s1 = Session(a_host, a_port, a_user, a_pass)
s2 = Session(b_host, b_port, b_user, b_pass)
...
def compare_point_avg_ts_parallel(ts_list):
    ts_list_parts = [ts_list[i:i + 10] for i in range(0, len(ts_list), 10)]
    for ts_list_part in ts_list_parts:
        with Pool() as p:
            p.map(compare_point_avg_ts_helper, ts_list_part)
...
def main():
    ...
    compare_point_avg_ts_parallel(ts_list)  # 对比时间序列
